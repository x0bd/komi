...
import { LuArrowLeft, LuBrain, LuFileUp, LuLoaderCircle, LuSparkles } from "react-icons/lu"
...
function parseResult(result: string | null): { winner: ReplayWinner; reason: ReplayReason } {
  if (!result) {
...
function buildKeyMoments(moves: Move[], size: 9 | 13 | 19) {
  ...
  const weighted = moves
    .map((move, index) => {
      ...
      const captures = nextState.captured[move.player] - state.captured[move.player]
      state = nextState
...
export function ReplayPageClient({ gameId }: { gameId: string }) {
  ...
  const [analysisByMove, setAnalysisByMove] = useState<Record<number, EngineAnalysis>>({})
  const [analysisFailedMoves, setAnalysisFailedMoves] = useState<Record<number, boolean>>({})
  const [tutorNotesByMove, setTutorNotesByMove] = useState<Record<number, string>>({})
  const [analysisLoadingMove, setAnalysisLoadingMove] = useState<number | null>(null)
  const [tutorLoadingMove, setTutorLoadingMove] = useState<number | null>(null)
    ...
    async function loadGame() {
      ...
      try {
        ...
        setDbGame({
          ...
          blackName: game.blackPlayer.name?.trim() || game.blackPlayer.email,
          whiteName: game.whitePlayer.name?.trim() || game.whitePlayer.email,
        ...
        const message =
          error instanceof Error ? error.message : "Unable to load replay game"
  ...
  const maxMove = replayFrames.length > 0 ? replayFrames.length - 1 : activeGame?.moves.length ?? 0
  const clampedMove = Math.max(0, Math.min(currentMove, maxMove))
  const currentFrame = replayFrames[clampedMove] ?? null
  const currentReplayMove = activeGame && clampedMove > 0 ? activeGame.moves[clampedMove - 1] : null
  ...
  useEffect(() => {
    ...
    if (analysisByMove[clampedMove] || analysisFailedMoves[clampedMove]) return
    ...
    async function loadAnalysis() {
      try {
        const response = await fetch("/api/analyze", {
          ...
          body: JSON.stringify({
            ...
            state: {
              ...
              consecutivePasses: currentFrame.state.consecutivePasses,
        ...
        if (!controller.signal.aborted) {
          setAnalysisLoadingMove((current) => (current === clampedMove ? null : current))
        }
      }
    }
  ...
  }, [activeGame, analysisByMove, analysisFailedMoves, clampedMove, currentFrame])
      ...
      try {
        const response = await fetch("/api/tutor", {
          ...
          body: JSON.stringify({
            ...
            lastMove: {
              coordinate,
              isPass: currentReplayMove.isPass,
            },
            analysis: {
              quality,
              winRate: analysis
              ...
              topMoves: analysis?.topMoves.slice(0, 3).map((candidate) => ({
        ...
        if (!controller.signal.aborted) {
          setTutorLoadingMove((current) => (current === clampedMove ? null : current))
        }
      }
    }
  ...
  const currentAnalysis = clampedMove > 0 ? analysisByMove[clampedMove] : undefined
  const keyMoments = useMemo(
    () => (activeGame ? buildKeyMoments(activeGame.moves, activeGame.size) : []),
    [activeGame],
  )
  ...
  const parsedResult = useMemo(() => parseResult(activeGame?.result ?? null), [activeGame?.result])
  ...
  const loadSgfFromText = () => {
    ...
    try {
      ...
      if (!size) {
        setSgfError("Only 9x9, 13x13, or 19x19 SGF files are supported right now.")
        return
      }
      ...
      const message = error instanceof Error ? error.message : "Invalid SGF"
      setSgfError(message)
    }
  }
        ...
        <Card className="w-full max-w-lg">
          ...
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">
              {gameError ?? "Unable to render this replay right now."}
            </p>
            <Button variant="secondary" render={<Link href="/games" />}>
  ...
  const currentTutorNote = clampedMove > 0 ? tutorNotesByMove[clampedMove] : null
        ...
        <GoBoard
          ...
          lastMove={
            currentFrame.lastMove && !currentFrame.lastMove.isPass
              ? { x: currentFrame.lastMove.x, y: currentFrame.lastMove.y }
              : undefined
          }
          onIntersectionClick={() => {}}
        />
      ...
      sidebar={
        <div className="flex flex-col gap-4 lg:h-full lg:min-h-0">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="flex flex-wrap items-center justify-between gap-2 text-base">
                <span className="font-display">Replay Review</span>
            ...
            <CardContent className="space-y-2 text-xs text-muted-foreground">
              <p>{activeGame.blackName} vs {activeGame.whiteName}</p>
              <p>{formatDate(activeGame.startedAt)} • {activeGame.moves.length} moves</p>
              <div className="flex flex-wrap gap-2 pt-1">
                <Button variant="outline" size="sm" render={<Link href="/games" />}>
                  <LuArrowLeft className="size-4" />
                  History
                </Button>
                {overrideGame ? (
                  <Button variant="secondary" size="sm" onClick={() => setOverrideGame(null)}>
                    Use database game
                  </Button>
                ) : null}
              </div>
          ...
          <MoveHistorySection
            ...
            highlightedMoveNumber={clampedMove > 0 ? clampedMove : undefined}
          ...
          <Card className="lg:flex-none">
            ...
            <CardContent className="space-y-2 text-sm">
              {clampedMove === 0 ? (
                <p className="text-muted-foreground">Move the timeline to start analysis.</p>
                ...
                <>
                  <p className="text-muted-foreground">{currentAnalysis.summary}</p>
                  <div className="rounded-xl border border-border/70 bg-secondary/20 p-3">
                    <p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
                      Best move
                    </p>
                    <p className="mt-1 font-mono text-sm font-semibold">
                      {currentAnalysis.recommendedMove.isPass
                        ? "Pass"
                        : toCoordinate(
                            {
                              x: currentAnalysis.recommendedMove.x,
                              y: currentAnalysis.recommendedMove.y,
                              player: currentFrame.state.turn,
                              isPass: false,
                            },
                            activeGame.size,
                ...
                <p className="text-muted-foreground">Analysis is unavailable for this move.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="lg:flex-none">
            ...
            <CardContent className="space-y-2 text-sm">
              {clampedMove === 0 ? (
                <p className="text-muted-foreground">Pick a move to see tutor commentary.</p>
                ...
                <p className="text-muted-foreground">No tutor note for this move yet.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="lg:flex-none">
            ...
            <CardContent className="space-y-3">
              <Textarea
                value={sgfText}
                onChange={(event) => setSgfText(event.target.value)}
              ...
              <Button variant="outline" size="sm" onClick={loadSgfFromText}>
                Load SGF replay
              </Button>
            </CardContent>
          </Card>
